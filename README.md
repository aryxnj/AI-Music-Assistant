...
It contains everything needed to **train, evaluate and serve** four melody-continuation models (Markov, custom LSTM, MelodyRNN, MusicVAE) plus two Streamlit front-ends:
...
| `packages.txt` | System packages consumed by the DevContainer |
...
Recreate the expected local tree as follows:

| Local folder (ignored) | Why it’s needed | How to obtain |
|------------------------|-----------------|---------------|
| **`magenta/magenta/`** | Google Magenta source (MelodyRNN / MusicVAE) | `git clone --depth 1 https://github.com/magenta/magenta.git magenta/magenta`<br/>`pip install -e magenta/magenta[extras]` |
| **`magenta/magenta_setup/`** | Utility scripts from Magenta | Included automatically after the clone above |
| **`checkpoints/`** | Pre-trained Magenta bundles & VAEs | `mkdir -p checkpoints && cd checkpoints`<br/>`wget https://storage.googleapis.com/magentadata/models/melody_rnn/attention_rnn.mag`<br/>`wget https://storage.googleapis.com/magentadata/models/melody_rnn/basic_rnn.mag`<br/>`wget https://storage.googleapis.com/magentadata/models/melody_rnn/lookback_rnn.mag`<br/>`wget https://storage.googleapis.com/magentadata/models/melody_rnn/mono_rnn.mag`<br/>`wget https://storage.googleapis.com/magentadata/models/music_vae/checkpoints/hierdec-mel_16bar.tar`<br/>`tar -xf hierdec-mel_16bar.tar && rm hierdec-mel_16bar.tar` |
| **`Fluid_R3_GM/`** & **`fluidsynth-2.4.0/`** | Optional local build of FluidSynth & GM sound-font | `sudo apt install fluidsynth fluid-soundfont-gm`<br/>or<br/>`brew install fluid-synth` |
| **`models/*/pickles/`** | Markov & LSTM training pickles from the _Mono-MIDI Transposition Dataset_ | `python models/markov/train_markov.py --download_dataset`<br/>`python models/lstm/train_lstm.py --download_dataset` |
| **`models/lstm/old-dataset/13369389/`** | 4-Bars Monophonic Melodies dataset (Zenodo 13369389) | `wget https://zenodo.org/record/13369389/files/13369389.zip?download=1 -O folk.zip`<br/>`unzip folk.zip -d models/lstm/old-dataset/13369389 && rm folk.zip` |

> **Tip:** run `git lfs install` first if you decide to track any of these files yourself.
